...
7. Run `cd frontend` and then `npm install` (in order to install necessary packages for the frontend side)
8. Run `cd ..` and then `npm run start` (in order to start the application)
9. Navigate to http://localhost:3000/
10. You can now browse our application :)
